...
{
    [SerializeField] private PlatformShape[] allPlatforms; // 모든 플랫폼 정보
    [SerializeField] private Transform lastPlatform; // 마지막 플랫폼 정보

    /// <summary>
    /// 플랫폼 생성 및 설정, 마지막 플랫폼 위치 설정
    /// </summary>
    public int SpawnPlatforms()
    {
        ...
        for(int i=0; i< platformCount; ++i)
        {
            ...
            // 매 5번째 플랫폼마다 50%의 확률로 180도 더 회전 (난이도 조절)
    ...
    private int SetupPlatformCount()
    {
        ...
        // baseCount * ((level+10)/10) : 10 스테이지 마다 baseCount 에 설정된 개수인 20개씩 증가
